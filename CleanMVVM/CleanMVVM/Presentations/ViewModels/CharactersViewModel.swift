...
final class CharactersViewModel: CharactersVMProtocol {
        ...
        fetchCharactersUseCase.execute(page: currentPage)
            .sink(receiveCompletion: { [weak self] completion in
                ...
                if newCharacters.isEmpty {
                    ...
                    let newViewModels = newCharacters.map { CharacterModel(from: $0) }
                    self.characters.append(contentsOf: newViewModels)
                    self.currentPage += 1
                }
            })
