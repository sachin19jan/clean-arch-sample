...
protocol FetchCharactersUseCaseProtocol {
    ...
    /// - Returns: A publisher that emits an array of `CharacterEntity` objects or an `Error` if the request fails.
    func execute(page: Int) -> AnyPublisher<[CharacterEntity], Error>
}
